...
class QuestionsDao:
    ...
    def create_question(self, topic: str, question: str, question_id: Optional[int]) -> Question:
        if question_id is not None:
            ...
            if db_item is not None:
                ...
                db_item = models.QuestionsModel(topic=topic, question_statement=question, question_id=question_id)
                self.db.add(db_item)
        else:
            db_item = models.QuestionsModel(topic=topic, question_statement=question)
            self.db.add(db_item)
        self.db.commit()
